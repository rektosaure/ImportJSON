...
test('Apps Script library bundle exposes IMPORTJSON', async () => {
  ...
  assert.match(bundle, /function IMPORTJSON\(url, query, columns, shape, cache, authorization\)/);
});
...
test('dist contains the small user-facing wrapper', async () => {
  ...
  assert.match(wrapper, /function IMPORTJSON\(url, query, columns, shape, cache, authorization\)/);
